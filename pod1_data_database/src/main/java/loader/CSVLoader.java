...
public class CSVLoader {
    ...
    public static LoadResult run(String dataFolder, String dbPath, PrintStream out) throws Exception {
        ...
        try {
            ...
            int patients = loadPatients(conn, dataFolder);
            int requests = loadRequests(conn, dataFolder);

            logActivity(conn, "Loaded " + (locations + roads + resources + patients + requests) + " records", "loader");

            long elapsed = System.currentTimeMillis() - start;
            recordTestResult(conn, "CSV full load", elapsed, "PASS");

            printSummary(out, dbPath, locations, roads, resources, patients, requests, elapsed);

            return new LoadResult(locations, roads, resources, patients, requests, elapsed);
    ...
    private static int loadPatients(Connection conn, String dataFolder) throws Exception {
        String sql = "INSERT INTO Patients (id, first_name, last_name, age, gender, location_id, condition) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (BufferedReader br = new BufferedReader(new FileReader(dataFolder + "patients.csv"));
             PreparedStatement ps = conn.prepareStatement(sql)) {
            br.readLine(); // skip header: id,first_name,last_name,age,gender,location_id,condition
            int count = 0;
            String line;
            while ((line = br.readLine()) != null) {
                if (line.trim().isEmpty()) continue;
                ...
                ps.setString(2, d[1].trim());
                ps.setString(3, d[2].trim());
                ps.setInt(4, Integer.parseInt(d[3].trim()));
                ps.setString(5, d[4].trim());
                ps.setInt(6, Integer.parseInt(d[5].trim()));
                ps.setString(7, d[6].trim());
                ps.executeUpdate();
                count++;
            }
            return count;
    ...
    private static int loadRequests(Connection conn, String dataFolder) throws Exception {
        String sql = "INSERT INTO Requests (id, patient_id, type, urgency_level, submitted_time, "
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(dataFolder + "requests.csv"));
            ...
            while ((line = br.readLine()) != null) {
                ...
                ps.setInt(2, Integer.parseInt(d[1].trim()));    // patient_id
                ps.setString(3, d[5].trim());                   // type
                ps.setInt(4, Integer.parseInt(d[6].trim()));    // urgency_level
                ps.setString(5, d[8].trim());                   // submitted_time
                ps.setInt(6, Integer.parseInt(d[3].trim()));    // origin_location_id
                ps.setInt(7, Integer.parseInt(d[4].trim()));    // destination_location_id
                ps.setString(8, d[7].trim());                   // status
                ps.executeUpdate();
                count++;
            }
            return count;
    ...
    private static void clearData(Connection conn) throws Exception {
        ...
        try (PreparedStatement ps = conn.prepareStatement("DELETE FROM Patients");) {
            ps.executeUpdate();
        }
    ...
    private static void printSummary(PrintStream out, String dbPath, int locations, int roads,
                                     int resources, int patients, int requests, long elapsed) {
        out.println();
        ...
        out.println("Patients  : " + patients);
        out.println("Requests  : " + requests);
        out.println("--------------------------------------------");
        out.println("Total     : " + (locations + roads + resources + patients + requests));
    ...
    public static class LoadResult {
        ...
        public final int patients;
        public final int requests;
        public final long elapsedMs;

        public LoadResult(int locations, int roads, int resources, int patients, int requests, long elapsedMs) {
